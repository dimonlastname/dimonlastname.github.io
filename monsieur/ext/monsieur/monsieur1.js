...
return \``;
            ...
            this.isVisible = function (HTMLElement){
                ...
                if ( (ContentStyle.display === "none") || (ContentStyle.opacity === "0") || (ContentStyle.visibility === "hidden") )
                    return false;
                //check for null-size
                if ( parseInt(ContentStyle.minWidth) === 0 && parseInt(ContentStyle.width) === 0)
                    return false;
                if ( parseInt(ContentStyle.minHeight) === 0 && parseInt(ContentStyle.height) === 0)
                    return false;
        ...
        ErrorHint(object, msg) {
            ...
            object.parentElement.append(div);
...
class MonsieurContent {
                ...
                parent = null)                         //link to parent MonsieurContent of SubContent
    {
        ...
        this.__private.Show = function(e) {
            ...
            let duration = parseInt(style.transitionDuration.replace('ms', '').replace(/(0.)?([0-9]+)s/, '$1$2'+"000"));
            let durationAni = parseInt(style.animationDuration.replace('ms', '').replace(/(0.)?([0-9]+)s/, '$1$2'+"000"));
...
class Templator{
    constructor(
...

        {
            ...
            ContentType = "untyped",  //TODO when refresh one of typed Templator, would be refreshed/added/removed all of same type Templators (exclude untyped ofcourse)
        ...
        } = {})
    {
        ...
        this._DataCount = DataCount > 0 ? DataCount : this.Data.length ;
        this._PageCursor = 0;
        ...
        const Refresher = function () {
            ...
            for (let i = 0; i < Dictionary.length; i++)
            {
                ...
                for (let j = 0; j < Dictionary[i].fields.length; j++)
                {
                    ...
                    let NewValue = Dictionary[i].fields[j].BuildValue($this.Data);
                    if (isAttribute)
                    {
                        ...
                        Dictionary[i].obj[Dictionary[i].fields[j].Target] = Dictionary[i].fields[j].BuildValue($this.Data);
                    }
        ...
        const Rebuilder = function () {
            ...
            for (let i = $this._PageCursor; i < Limit; i++)
                lines += $this._LineBuilder($this.Data[i], i, $this.Data.length);
            //appending
            //   debugger;
            if ($this.Content.children.length < 1)
            {
                ...
                lines = Monsieur.CreateElementFromString(lines, $this.Content.tagName);
                $this.Content.append(lines);
            }
            ...
            if ($this.Data.length === 0 && $this.EmptyMessage !== "" && !$this.isEmptyHide)
            {
            ...
            else if ($this.Data.length === 0 && $this.isEmptyHide){
                $this.Content.style.display = 'none';
            }
            ...
            if ($this.PageSize > 0 && $this._PageCursor < $this._DataCount){
                ...
                btnNext.onclick = function(){
                    if ($this._PageCursor >= $this.Data.length)
        ...
        const Build = function () {
            ...
            if ($this.Type === "Refresh")
                Refresher();
            else if ($this.Type === "ItemList" && $this.Data.length > 0)
                Rebuilder();
        ...
        this.Refresh = function(data = null){
        ...
        this.Add = function(item, extraclass = false, isPrepend = false){
            ...
            if (!isPrepend)
            {
                elem.innerHTML = $this._LineBuilder(item, $this.Data.length, $this.Data.length+1);
                while (elem.childNodes[0]) {
                ...
                elem.innerHTML =  $this._LineBuilder(item, 0, $this.Data.length+1);
                while (elem.childNodes[0]) {
                    fragment.appendChild(elem.childNodes[0]);
                }
                if (extraclass)
    ...
    )
    {
        ...
        if (this.SubSelector === null){
            ...
            Branch = Branch.replace(/^([\s\S]*)(<\/\w+>)$/, function (match, html, entag) {
                entag = `<div class="mtb-sub_tree"></div>${entag}`;
                return html+entag
            })
        }
     ...
     //   this.LineBuilder = LineBuilder;
            ...
            for (let key in obj)
            {
                ...
                if (Array.isArray(ObjItem))
                {
                    ...
                    for (let i = 0; i < ObjItem.length; i++)
                    {
                        if ($this.SubSelector === null)
                        {
                            line.append(  BuildElement(ObjItem[i], key, i) );
                        }
                        else
                        {
                            line.classList.add('mtb-has_tree');
                            let sub = line.querySelector($this.SubSelector);
                            sub.classList.add('mtb-sub_tree');
                            sub.append(  BuildElement(ObjItem[i], key, i) );
                        }
                    }
                    Lvl--;
                }
        ...
        let Build = function () {
            ...
            if (Array.isArray($this.Data))
            {
                ...
                for (let j = 0; j < $this.Data.length; j++)
                    $this.Content.append(  BuildElement($this.Data[j], "root", 0) );
                Lvl--;
            }
            else{
                $this.Content.append(BuildElement($this.Data, "root", 0));
            }
            Lvl = 0;
            Index = 0;
            $this.AfterBuild();
        ...
        //### METHODS
        this.Refresh = function (data=$this.Data) {
            $this.Data = data;
            Build();
        }
...
class MonsieurTutor{
    constructor(TargetButton = null, TargetContent = null){
       ...
       //     document.body.style.position = '';
        ...
        document.body.append(this.ContentBG);
